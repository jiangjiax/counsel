...
```bash
cp .env.example .env   # 如果有示例文件；或直接新建
```

填入以下三个参数：

```env
CODEANY_API_KEY=your-api-key
...
| 变量 | 说明 |
|------|------|
| `CODEANY_API_KEY` | API 密钥 |
| `CODEANY_BASE_URL` | API 地址，支持任意 Anthropic 兼容接口 |
| `CODEANY_MODEL` | 模型名称 |

> 项目底层使用 Anthropic 兼容格式，可接入 Anthropic 原生 API 或其他兼容服务（如 MiniMax）。
...
> 若端口被占用，服务会自动尝试 3001–3005，实际端口见终端输出。
...
![Step 0 · 用户放心倾诉困惑](examples/0.用户放心的说自己的困惑.png)

**Step 1 · Facilitator 将困惑翻译成可讨论的问题**

![Step 1 · 把用户的困惑翻译成可以讨论的问题](examples/1.把用户的困惑翻译成可以讨论的问题.png)

**Step 2 · 每位幕僚深度挖掘背后事实**

![Step 2 · 每位幕僚深度的挖掘背后的事实](examples/2.每位幕僚深度的挖掘背后的事实.png)

**Step 3 · 每位幕僚独立分享自己的观点**

![Step 3 · 每位幕僚独立分享自己的观点](examples/3.每位幕僚独立分享自己的观点.png)

**Step 4 · 把问题拆解成多个面、多个视角、多层**

![Step 4 · 把问题拆解成多个面、多个视角、多层](examples/4.把问题拆解成多个面、多个视角、多层.png)

**Step 5 · 对每个维度深度辩论**

![Step 5 · 对每个维度深度辩论-正反、底层、融合](examples/5.对每个维度深度辩论-正反、底层、融合.png)

**Step 6 · 汇总：共识、冲突、贝叶斯迭代**

![Step 6 · 汇总～共识、冲突、贝叶斯迭代](examples/6.汇总～共识、冲突、贝叶斯迭代.png)

**Step 7 · 摘果子**

![Step 7 · 摘果子：todo, 幕僚对用户评价，各自收获](examples/7.摘果子：todo,%20幕僚对用户评价，各自收获.png)
...
**蒋鲤**  
技术合伙人，负责多 Agent 并行架构和系统工程。将复杂的 AI 协作机制转化为可运行、可复现的工程实现。
