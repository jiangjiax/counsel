...
/// Buffer size for SSE chunks (in BYTES, not chars) — flushes when the
/// accumulated buffer exceeds this. Lower = smoother streaming (more deltas,
/// finer granularity). 2026-04-25: dropped 500 → 80 (~26 Chinese chars per
/// chunk) so Step 5/7/8 facilitator + Step 4/6/8a persona streams flow
/// smoothly instead of jumping in 166-char bursts. The break-point logic
/// still tries to land on punctuation/newlines so chunks read naturally.
pub const BUFFER_THRESHOLD: usize = 80;
