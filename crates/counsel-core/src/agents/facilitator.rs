...
impl Agent for FacilitatorAgent {
    ...
    async fn run_streaming_collect(
        ...
        // Empty-response guard. Mirrors PersonaAgent::run_streaming_collect
        // (agents/mod.rs:249). Without this, an empty facilitator response
        // (provider content filter, oversized context, etc.) silently writes
        // an empty 01-defined.md and the frontend only catches it via its
        // silent-failure detection — too late for incident logging.
        if full_text.trim().is_empty() {
            sender.send(SSEEvent::Error {
                message: "facilitator returned 0 chars (likely context too long or provider refusal)".into(),
            }).await.ok();
            return Err(super::AgentError::Generic(
                "Model returned empty response".into(),
            ));
        }
