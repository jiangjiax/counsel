...
impl CounselService {
    ...
    pub async fn run_define(
        ...
        let user_wiki = self.storage.read_user_wiki().await.unwrap_or_default();
        // Phase 4.3 — Session N+1 auto-inherit: extract the most recent session's
        // wiki entry so the facilitator can recognize continuity with prior sessions.
        let last_session = self.storage.read_last_wiki_entry().await.unwrap_or_default();
        // Phase 2.8 — belief-system.md: structured project-level prior (latest
        // Bayesian posterior). Fed into facilitator prompts alongside last_session.
        ...
        tracing::info!(
            ...
            user_wiki_chars = user_wiki.len(),
            last_session_chars = last_session.len(),
            prior_beliefs_chars = prior_beliefs.len(),
            execution_journal_chars = execution_journal.len(),
            total_chars = raw_input.len() + user_wiki.len() + last_session.len() + prior_beliefs.len() + execution_journal.len(),
            "run_define context sizes (pre-cap)"
        );
        ...
        let (final_response, new_state) = match (&current_state, &user_input) {
            ...
            (DefineState::Init, None) => {
                let messages = vec![
                    ...
                    ChatMessage::user(facilitator_define_prompt(&raw_input, &user_wiki, &last_session, &prior_beliefs, &execution_journal)),
                ];
            ...
            (DefineState::Confirming { attempt: 1, draft }, Some(correction)) => {
                let messages = vec![
                    ChatMessage::system("你是私董会的主持人，帮助案主锁定核心问题。"),
                    ChatMessage::user(facilitator_correction_prompt(&raw_input, draft, correction, &user_wiki, &last_session, &prior_beliefs, &execution_journal)),
                ];
    ...
    pub async fn run_facts(
        ...
        for wp in self.registry.all() {
            let id = &wp.slug;
            let name = &wp.name;
            let title = &wp.title;
            let desc = wp.short_description();
            let base_prompt = wp.build_system_prompt(&raw_input, Some(&fp));
            let system_prompt = rich_persona_facts_prompt(&base_prompt, &raw_input, &defined);

            let prompt = facts_user_prompt(&raw_input, &defined, "");
            let messages = vec![
            ...
            let persona = PersonaAgent::new(id, name, title, &desc, self.model.clone());
            let question_response = persona.run(&messages, ChatOptions::default().max_tokens(200)).await
                .map_err(|e| crate::CoreError::Agent(e.to_string()))?;

            let completion_tokens = estimate_tokens_static(&question_response);
            total_tokens += completion_tokens;

            let question_text = question_response.trim();
            if question_text.to_uppercase() == "N/A" || question_text.is_empty() {
                continue;
            }

            // Send question event
            sender.send(SSEEvent::PersonaStart { name: name.to_string() }).await
                .map_err(|e| crate::CoreError::Channel(e.to_string()))?;
            sender.send(SSEEvent::PersonaChunk { name: name.to_string(), chunk: question_text.to_string() }).await
                .map_err(|e| crate::CoreError::Channel(e.to_string()))?;
            sender.send(SSEEvent::PersonaDone { name: name.to_string() }).await
                .map_err(|e| crate::CoreError::Channel(e.to_string()))?;
...
Provide a clear, direct answer in 2-3 sentences."#,
                    name, question_text, raw_input, defined
                );
                ...
                all_qa.push_str(&format!("\n\n## {} Questions\n{}\n\n## Client's Answer\n{}\n", name, question_text, user_answer.trim()));
            } else {
                // User mode: save questions only, user will answer later
    ...
    pub async fn run_opinions(
        ...
        for wp in self.registry.all() {
            ...
            join_set.spawn(async move {
                ...
                let messages = vec![
                    ChatMessage::system(&system_prompt),
                    ChatMessage::user(opinion_prompt),
                ];
                ...
                let result = persona.run_streaming_collect(
                    &messages,
                    ChatOptions::default().max_tokens(1200).temperature(0.8),
                    sender.into(),
    ...
    pub async fn run_dimensions(
        ...
        let prompt_tokens = estimate_messages_tokens(&messages);
        let response = secretary.run(&messages, ChatOptions::default().max_tokens(1000)).await
            .map_err(|e| crate::CoreError::Agent(e.to_string()))?;
    ...
    pub async fn run_debate(
        ...
        // ========== SYNTHESIS: Secretary synthesizes R1 + Middle (no Round 2) ==========
        // Use streaming so the frontend receives FacilitatorChunk events (prevents idle timeout)
        let secretary = SecretaryAgent::new(self.model.clone());
        let synthesis_prompt = debate_round1_synthesis_prompt(
            &dimension.name,
            &all_round1_positions,
            &all_middle_responses,
        );
        let messages = vec![
            ChatMessage::system("You are a skilled debate facilitator."),
            ChatMessage::user(synthesis_prompt),
        ];

        let synthesis = secretary.run_streaming_collect(
            &messages,
            ChatOptions::default().max_tokens(1600),
            sender.clone().into(),
        ).await.map_err(|e| crate::CoreError::Agent(e.to_string()))?;

        // Append to debate record
        ...
        let total_tokens = estimate_tokens_static(&all_round1_positions)
            ...
            + estimate_tokens_static(&synthesis);
        let metrics = StepMetrics {
            step: 6,
    ...
    pub async fn run_premortem(
        ...
        // max_tokens=1600 (~800 Chinese chars). Michael 2026-04-22: "限制字数
        // 让阅读体验更好，点到为止比穷举更有力". Prompt also explicitly caps at
        // 800 汉字 as a soft budget hint. Timeout can return to 30s now.
    ...
    pub async fn run_summary(
        ...
        // Pre-mortem may or may not exist (gracefully degrades if skipped)
        let premortem = self.storage.read_session_file(project_id, session_id, "premortem.md").await.unwrap_or_default();
...
{}"#,
        ...
        let response = secretary.run_streaming_collect(
            &messages,
            ChatOptions::default().max_tokens(3000),
            sender.clone().into(),
        ).await.map_err(|e| crate::CoreError::Agent(e.to_string()))?;
        let completion_tokens = estimate_tokens_static(&response);
        let duration_ms = start_time.elapsed().as_millis() as u64;
    ...
    pub async fn run_harvest(
        ...
        for wp in self.registry.all() {
            ...
            join_set.spawn(async move {
                ...
                let prompt_tokens = estimate_messages_tokens(&messages);
                let result = agent.run(&messages, ChatOptions::default().max_tokens(600)).await
                    .map_err(|e| crate::agents::AgentError::Generic(e.to_string()))?;
                let completion_tokens = estimate_tokens_static(&result);
        ...
        // Step 8b: Extract todos with persona comments and achievements
        ...
        // Progress: tail phase (todos + bayesian) takes 30-90s combined, non-streaming.
        // Emit a facilitator chunk so the user sees activity rather than a frozen UI.
        ...
        let todo_prompt_tokens = estimate_messages_tokens(&messages);
        let todo_response = secretary.run(&messages, ChatOptions::default().max_tokens(1000)).await
            .map_err(|e| crate::CoreError::Agent(e.to_string()))?;
        let todo_completion_tokens = estimate_tokens_static(&todo_response);

        sender.send(SSEEvent::FacilitatorChunk {
            chunk: "✓ 行动清单已提炼。正在更新贝叶斯信念…\n\n".to_string()
        }).await.ok();

        // Step 8c: Bayesian belief update — now takes client's self-reflection AND
        // real-time resonance signals as parallel evidence streams alongside advisor
        // evaluations (Phase 2.13 + 2.7 product decisions, 2026-04-22).
        ...
        let bayesian_response = secretary.run(&bayesian_messages, ChatOptions::default().max_tokens(2400)).await
            .map_err(|e| crate::CoreError::Agent(e.to_string()))?;
...
pub fn parse_dimensions(response: &str) -> Vec<Dimension> {
    ...
    for line in response.lines() {
        ...
        if line.starts_with("## ") && !line.to_lowercase().contains("unanimous") {
            ...
            if line_lower.starts_with("core conflict:") {
